	...
	for {
		...
		if err != nil {
			if errors.Is(err, context.Canceled) {
				c.opts.logger.Log(log.LevelInfo, "watcher's ctx cancel : %v", err)
				return
			}
			time.Sleep(time.Second)
			c.opts.logger.Log(log.LevelInfo, "failed to watch next config: %v", err)
			continue
		}
...
func (c *config) Load() error {
	for _, src := range c.opts.sources {
		...
		for _, v := range kvs {
			c.opts.logger.Log(log.LevelInfo, "config loaded: %s format: %s", v.Key, v.Format)
		}
		if err = c.reader.Merge(kvs...); err != nil {
